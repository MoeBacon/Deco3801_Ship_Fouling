...
from app.schemas import FrameResponse, DetectionResponse

router = APIRouter()

# Returns all extracted frames for a video, called by React after polling --> status --> done
...
def get_frames(video_id: str):
    ...
    frames = []
    for row in rows:
        # Converts file path on disk to a URL React can fetch
        # e.g. frames/abc123/frame_0001.jpg -> /static/abc123/frame_0001.jpg
        image_url = "/" + row["file_path"].replace("frames/", "static/", 1).replace("\\", "/")
...
# Returns ML detections for a specfic frame, called by react when operator (clicks?) on frame  
@router.get("/frames/{frame_id}/detections", response_model=list[DetectionResponse])
def get_detections(frame_id: str):
    connection = get_connection()
    cursor = connection.cursor()

    # Check frame exists first
