...
export default function DashboardPage() {
  const [lastImport, setLastImport] = useState<ImportInspectionResponse | null>(null)

  useEffect(() => {
    try {
      const raw = window.sessionStorage.getItem(uploadDraftStorageKey)
      if (!raw) return
      const parsed = JSON.parse(raw) as { result?: ImportInspectionResponse | null }
      setLastImport(parsed.result ?? null)
    } catch {
      setLastImport(null)
    }
  }, [])
          ...
          {lastImport ? (
            <div className="mt-4 rounded-lg border border-border bg-surface-0/60 p-4">
              ...
              <p className="mt-2 text-sm text-white">
                Video ID: <span className="font-medium">{lastImport.video_id}</span>
              </p>
