...
export default function AnalysisPage() {
  ...
  const imported =
    (location.state as { import?: ImportInspectionResponse } | null)?.import ??
    null;
  ...
  const [frameLoadError, setFrameLoadError] = useState<string | null>(
    !videoId && !imported
      ? "No analysis context found. Start from Upload Footage and run an import first."
      : null,
  );
  ...
  const selectedFrame = useMemo(() => {
    ...
    return (
      frames.find((frame) => frame.frame_id === selectedFrameId) ?? frames[0]
    );
  ...
  useEffect(() => {
    ...
    const preloadDetections = async () => {
      for (const frame of frames) {
        ...
        inFlightDetectionsRef.current.add(cacheKey);
        try {
          const data = await getFrameDetections(
            frame.frame_id,
            controller.signal,
          );
          if (cancelled) return;
  ...
  const groupedFrameDetections = useMemo(() => {
    ...
    const allDetections = frames.flatMap(
      (frame) =>
        detectionsByFrame[`${analysisContextKey}:${frame.frame_id}`] ?? [],
    );
    for (const detection of allDetections) {
      counts.set(
        detection.class_label,
        (counts.get(detection.class_label) ?? 0) + 1,
      );
    }
  ...
  const frameGraphItems = useMemo(() => {
    return frames.map((frame) => {
      ...
      const frameDetections =
        frame.frame_id === activeFrameId ? visibleDetections : cached;
      const count = frameDetections?.length ?? 0;
      const status: "unknown" | "none" | "detected" =
        frameDetections === undefined
          ? "unknown"
          : count === 0
            ? "none"
            : "detected";
      ...
      for (const detection of frameDetections ?? []) {
        byLabel.set(
          detection.class_label,
          (byLabel.get(detection.class_label) ?? 0) + 1,
        );
      }
  ...
  const timelineShortcuts = useMemo(() => {
    ...
    for (let i = 0; i < frameGraphItems.length; i += 1) {
      ...
      if (currentStart >= 0 && closing) {
        ...
        if (!longestNone || len > longestNone.len)
          longestNone = { start: currentStart, end, len };
        currentStart = -1;
      }
    }
  ...
  const timelineGeometry = useMemo(() => {
    ...
    return frameGraphItems.map((item, idx) => {
      const x =
        frameGraphItems.length === 1 ? (left + right) / 2 : left + idx * step;
      const y =
        chartBottom - (item.count / timelineMaxCount) * (chartBottom - top);
      return { x, y, item };
    });
  ...
  const vesselName =
    imported?.vessel.vessel_name || (videoId ? `Video ${videoId}` : "Analysis");
  ...
  const collectDetectionsForExport = async (): Promise<
    Record<string, DetectionResponse[]>
  > => {
      ...
      {imported?.ok ? (
        <div className="rounded-lg border border-status-low/30 bg-status-low/10 px-4 py-3 text-sm text-slate-100">
          ...
          <span className="text-muted">
            {imported.frames.length} extracted frame(s)
          </span>
        ...
        actions={
          <>
            <button
              ...
              className="inline-flex items-center justify-center gap-2 rounded-lg border border-input-border bg-input-bg px-4 py-2 text-sm font-medium text-input-text hover:bg-surface-1 disabled:opacity-50 transition-colors"
            >
            ...
            <button
              ...
              className="inline-flex items-center justify-center gap-2 rounded-lg bg-accent px-4 py-2 text-sm font-semibold text-white hover:bg-accent-hover disabled:opacity-50 transition-colors"
            >
              {generatingReport ? (
                <>
                  <svg
      ...
      <div className="grid grid-cols-1 items-start gap-6 xl:grid-cols-3">
          ...
          <div className="mt-4 grid gap-4 lg:grid-cols-2">
            <div className="relative min-h-[16rem] overflow-hidden rounded-lg border border-border bg-surface-0">
              <div className="absolute left-3 top-3 rounded bg-black/50 px-2 py-1 text-xs text-white">
                {selectedFrame?.annotated_image_url
                  ? "Annotated frame"
                  : "Selected frame"}
              </div>
              {selectedFrame ? (
                <img
                  src={
                    selectedFrame.annotated_image_url ?? selectedFrame.image_url
                  }
              ...
              <div className="h-full overflow-y-auto p-4 pt-10">
                {loadingDetections ? (
                  ...
                  <ul className="space-y-2">
                    {visibleDetections.map((detection) => (
                      <li
                        ...
                        className="rounded border border-border bg-surface-1/80 px-3 py-2 text-sm"
                      >
                  ...
                  <div className="rounded-lg border border-status-mod/40 bg-status-mod/10 px-3 py-2 text-sm text-amber-100">
                    No detections were returned for this frame. This can mean
                    either nothing was detected, or ML inference is unavailable
                    in the current backend environment.
                  </div>
                )}
              </div>
            </div>
              ...
              {frames.map((frame) => (
                ...
                >
                  ...
                  <div className="px-2 py-1 text-xs text-placeholder">
                    #{frame.frame_number} ·{" "}
                    {formatTimestamp(frame.timestamp_in_video)}
                  </div>
                </button>
              ))}
              {!frames.length ? (
                <p className="text-sm text-muted">
                  No frames were returned by the backend.
                </p>
              ) : null}
            </div>
          </div>

          <div className="mt-6 rounded-lg border border-input-border bg-input-bg/50 p-4">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <h3 className="text-sm font-semibold text-white">
                Detection timeline
              </h3>
              <p className="text-xs text-muted">
                Click any point to jump to a frame and inspect details.
              </p>
            </div>
            ...
            <p className="mt-2 text-xs text-muted">
              Timeline loaded: {loadedDetectionFramesCount}/{frames.length || 0}{" "}
              frames
            </p>
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {timelineShortcuts.map((shortcut) => (
                  <button
                    ...
                    className="rounded-full border border-input-border bg-input-bg px-3 py-1 text-xs text-input-text hover:bg-surface-2 transition-colors"
                  >
                    {shortcut.label}
                  </button>
                ))}
            ...
            <div className="relative mt-4 overflow-visible rounded-lg border border-input-border bg-input-bg/50 p-3">
              <svg
                viewBox="0 0 1000 176"
                className="h-44 w-full"
                role="img"
                aria-label="Detection trend across frames"
              >
                <line
                  x1="24"
                  y1="132"
                      ...
                      onMouseEnter={() =>
                        setHoveredPoint({
                          ...
                          timestamp: formatTimestamp(
                            item.frame.timestamp_in_video,
                          ),
                        })
                      }
                      onMouseLeave={() =>
                        setHoveredPoint((current) =>
                          current?.frameId === item.frame.frame_id
                            ? null
                            : current,
                        )
                      }
                ...
                {timelineGeometry
                  .filter(
                    (_, idx) =>
                      idx %
                        Math.ceil(Math.max(1, timelineGeometry.length / 8)) ===
                      0,
                  )
                  .map(({ x, item }) => (
                    <text
                      key={`t-${item.frame.frame_id}`}
                      x={x}
                      y="158"
                      textAnchor="middle"
                      fontSize="11"
              ...
              {hoveredPoint ? (
                ...
                >
                  <p className="font-medium text-white">
                    Frame #
                    {
                      frames.find((f) => f.frame_id === hoveredPoint.frameId)
                        ?.frame_number
                    }
                  </p>
            ...
            {selectedFrame ? (
              <p className="mt-3 text-xs text-muted">
                Selected frame #{selectedFrame.frame_number}:{" "}
                {
                  frameGraphItems.find(
                    (item) => item.frame.frame_id === selectedFrame.frame_id,
                  )?.dominantLabel
                }
              </p>
            ) : null}
        ...
        <article className="rounded-xl border border-border bg-surface-1 p-6">
          ...
          <p className="mt-1 text-sm text-muted">
            Class totals across all frames in this analysis.
          </p>
          <div className="mt-5 space-y-3">
            {groupedFrameDetections.map(([label, count]) => {
              const pct = totalDetectionCount
                ? Math.round((count / totalDetectionCount) * 100)
                : 0;
              return (
                <div key={label}>
