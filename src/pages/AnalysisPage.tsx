...
export default function AnalysisPage() {
  ...
  const timelineShortcuts = useMemo(() => {
    ...
    const highest =
      withDetections.length > 0
        ? withDetections.reduce((best, cur) => (cur.count > best.count ? cur : best), withDetections[0])
        : null
    const firstBad = withDetections.length > 0 ? withDetections[0] : null

    let longestNone: { start: number; end: number; len: number } | null = null
    let currentStart = -1
    for (let i = 0; i < frameGraphItems.length; i += 1) {
      const isNone = frameGraphItems[i].status === 'none'
    ...
    if (longestNone && longestNone.len >= 3) {
      ...
      shortcuts.push({
        ...
        frameId: frameGraphItems[mid].frame.frame_id,
      })
    }
    return shortcuts
  }, [frameGraphItems])
