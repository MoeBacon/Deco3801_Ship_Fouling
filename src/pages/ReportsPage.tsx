...
function computeSummary(report: ReportData) {
  const allDetections: DetectionResponse[] = Object.values(
    report.detectionsByFrame,
  ).flat();
  ...
  for (const d of allDetections) {
    countsByLabel.set(
      d.class_label,
      (countsByLabel.get(d.class_label) ?? 0) + 1,
    );
  ...
  const framesWithDetections = Object.values(report.detectionsByFrame).filter(
    (d) => d.length > 0,
  ).length;
  const coveragePct =
    report.frames.length > 0
      ? Math.round((framesWithDetections / report.frames.length) * 100)
      : 0;
...
function frameHullSectionLabel(
  frame: FrameResponse,
  allFrames: FrameResponse[],
): string {
...
function computeCategoryHighlights(report: ReportData): CategoryHighlight[] {
  ...
  for (const [classLabel, detections] of byLabel) {
    const peak = detections.reduce((best, d) =>
  ...
  return highlights.sort(
    (a, b) => b.peakDetection.confidence - a.peakDetection.confidence,
  );
}
...
function getVideoDurationForSummary(report: ReportData): {
  value: string;
  hint: string;
} {
  const sec =
    report.frames.length > 0
      ...
      : typeof report.job.duration === "number" &&
          Number.isFinite(report.job.duration) &&
          report.job.duration > 0
        ? report.job.duration
        : null;
...
function HullDiagram({
  ...
  const sectionRates = useMemo(() => {
    ...
    return Array.from({ length: n }, (_, s) => {
      ...
      const withDets = slice.filter(
        (f) => (detectionsByFrame[f.frame_id] ?? []).length > 0,
      ).length;
    ...
    <div className="report-hull-schematic rounded-lg border border-slate-200 bg-slate-50 p-4">
      <svg viewBox="0 0 600 185" className="w-full" style={{ height: "14rem" }}>
        ...
        {SECTION_BOUNDS.map(({ x, w }, i) => {
          ...
          if (rate === 0) return null;
          const color =
            rate < 0.25 ? "#22c55e" : rate < 0.55 ? "#f59e0b" : "#ef4444";
      ...
      {hasAnyFouling ? (
        ...
        <p className="mt-2 text-center text-xs text-slate-400">
          No fouling detected across hull sections.
        </p>
      )}
    </div>
  );
...
function DetectionSummary({ highlights }: { highlights: CategoryHighlight[] }) {
  ...
  return (
    <div className="report-print-section border-b border-slate-200 px-8 py-6">
      <h2 className="mb-1 text-xs font-bold uppercase tracking-widest text-accent">
        Detection Summary
      </h2>
      <p className="mb-6 text-sm text-slate-500">
        Strongest match per fouling type: the frame where the model was most
        confident for each category.
      </p>
              ...
              <div className="space-y-4 p-5">
                <div>
                  ...
                  <p className="mt-0.5 text-sm text-slate-500">
                    Highest-confidence detection for this type
                  </p>
                </div>
    ...
    <div className="flex min-w-0 flex-col gap-6 pb-10">
      ...
      <section className="rounded-xl border border-border bg-surface-1 p-8 text-center">
        ...
        <p className="mt-2 text-sm text-muted">
          Complete an analysis run and click &ldquo;Generate report&rdquo; on
          the Analysis page.
        </p>
        <button
          type="button"
...
export default function ReportsPage() {
  ...
  const report = useMemo<ReportData | null>(() => {
    const stateReport = (location.state as { report?: ReportData } | null)
      ?.report;
    return stateReport ?? loadReport();
  }, [location.state]);

  const summary = useMemo(
    () => (report ? computeSummary(report) : null),
    [report],
  );

  const detectedFrames = useMemo(() => {
    if (!report) return [];
    return report.frames.filter(
      (frame) => (report.detectionsByFrame[frame.frame_id] ?? []).length > 0,
    );
  }, [report]);
  ...
  const shortId = (
    report.video_id.split("-")[0] ?? report.video_id
  ).toUpperCase();
  ...
  return (
    <div className="flex min-w-0 flex-col gap-6 pb-10">
        ...
        actions={
          <>
            ...
            <button
              ...
              className="no-print inline-flex items-center gap-2 rounded-lg bg-accent px-4 py-2 text-sm font-semibold text-white hover:bg-accent-hover transition-colors"
            >
              <svg
                className="size-4"
                fill="none"
      ...
      <div
        id="report-document"
        className="mx-auto w-full overflow-hidden rounded-xl border border-slate-300 bg-white text-slate-900 shadow-xl"
      ...
      >
            ...
            {report.file.client_filename ? (
              <p className="mt-1 text-slate-400">
                ...
                {report.file.size_bytes
                  ? ` (${formatBytes(report.file.size_bytes)})`
                  : ""}
              </p>
            ) : null}
          </div>
          ...
          <div className="grid grid-cols-2 gap-x-12 gap-y-2.5 text-sm">
            {[
              ...
              { label: "Location / Port", value: report.vessel.location },
              {
                ...
                value: report.vessel.operator_name,
              },
            ].map(({ label, value }) => (
              <div key={label} className="flex gap-3">
                <span className="w-36 shrink-0 font-medium text-slate-500">
                  {label}
                </span>
        ...
        <div className="report-print-section border-b border-slate-200 bg-slate-50 px-8 py-8">
          ...
          <p className="mt-1 max-w-2xl text-sm text-slate-500">
            Overview of footage processed and fouling detected during this
            inspection.
          </p>
          ...
          <div className="mt-8 space-y-6">
            <div>
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                {[
                  {
                    value: videoDurationSummary.value,
                    label: "Footage duration",
                    hint: videoDurationSummary.hint,
                  },
                  {
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                <div className="report-print-card rounded-xl border border-slate-200 bg-white px-6 py-5 shadow-sm">
                  ...
                  <p className="mt-2 text-xs text-slate-400">
                    Individual fouling instances identified
                  </p>
                </div>
                <div
                ...
                <div className="report-print-card rounded-xl border border-slate-200 bg-white px-6 py-5 shadow-sm">
                  ...
                  <p className="mt-2 text-xs text-slate-400">
                    Mean model confidence across all detections
                  </p>
                </div>
              </div>
            </div>
                ...
                {sortedLabels.length > 0 ? (
                  sortedLabels.map(([label, count], i) => {
                    ...
                    const avgConf = confList.length
                      ? confList.reduce((s, c) => s + c, 0) / confList.length
                      : 0;
        ...
        <div className="report-print-section border-b border-slate-200 px-8 py-6">
          <h2 className="mb-4 text-xs font-bold uppercase tracking-widest text-accent">
            Hull Location Schematic
          </h2>
          <HullDiagram
            frames={report.frames}
            detectionsByFrame={report.detectionsByFrame}
          />
        </div>
        ...
        {detectedFrames.length > 0 ? (
          <div className="report-print-section report-print-allow-break border-b border-slate-200 px-8 py-6">
            ...
            <p className="mb-4 text-sm text-slate-500">
              {detectedFrames.length} of {report.frames.length} analysed frames
              with fouling detections
            </p>
          ...
          <div className="border-b border-slate-200 px-8 py-6">
            ...
            <p className="text-sm text-slate-500">
              No frames with fouling detections for this inspection.
            </p>
          </div>
        ) : null}
