...
export default function DashboardPage() {
  ...
  const latestFrames = lastImport?.frames.length ?? lastImport?.job.frame_count ?? 0
  const footageDurationSec = lastImport?.frames.length
    ? Math.max(...lastImport.frames.map((frame) => frame.timestamp_in_video))
    : null
  ...
  const vesselName = lastImport?.vessel.vessel_name?.trim() || 'Unknown vessel'
  const inspectionDate = lastImport?.vessel.inspection_date?.trim() || 'Date not set'
  const operatorName = lastImport?.vessel.operator_name?.trim() || null
  const location = lastImport?.vessel.location?.trim() || null
    ...
    <div className="flex min-h-full min-w-0 flex-1 flex-col gap-6">
      ...
      <div className="grid shrink-0 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <article className="rounded-xl border border-border bg-surface-1 p-5 shadow-sm">
          ...
          <p className="mt-2 text-3xl font-semibold text-white">{lastImport?.job.status ?? 'N/A'}</p>
        ...
        <section className="flex min-w-0 flex-col rounded-xl border border-border bg-surface-1 p-6 lg:col-span-2">
          ...
          {lastImport ? (
            <div className="mt-4 rounded-lg border border-border bg-surface-0/60 p-4">
              ...
              <p className="mt-1 text-sm text-muted">
                Status: {lastImport.job.status} · Frames: {lastImport.frames.length}
              ...
              <div className="mt-4 flex gap-3">
                ...
                <Link
                  className="rounded-lg bg-accent px-3 py-2 text-sm font-semibold text-white hover:bg-accent-hover"
                  to={ROUTES.analysisByVideo(lastImport.video_id)}
                  state={{ import: lastImport }}
                >
                  Open Analysis
                </Link>
              </div>
            ...
            <div className="rounded-lg border border-border bg-surface-0/60 p-3">
              ...
              <p className="mt-1 text-white">
                {hasActiveImport
                  ...
                  : lastImport
                    ? `Latest import ${lastImport.job.status}`
                    : 'No recent import'}
              </p>
            </div>
          ...
          <div className="mt-5 grid grid-cols-1 gap-2">
            ...
            <Link
              className={`rounded-lg px-3 py-2 text-center text-sm font-semibold text-white ${
                lastImport ? 'bg-accent hover:bg-accent-hover' : 'bg-surface-2 opacity-60'
              }`}
              to={lastImport ? ROUTES.analysisByVideo(lastImport.video_id) : ROUTES.analysis}
              state={lastImport ? { import: lastImport } : undefined}
            >
