...
export default function DashboardPage() {
  const [lastImport, setLastImport] = useState<ImportInspectionResponse | null>(
    () => readUploadDraft()?.result ?? null,
  );
  const [history, setHistory] = useState<ImportInspectionResponse[]>(() =>
    readUploadHistory(),
  );
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeLocalStoreUpdates((key) => {
      if (
        key === IMPORT_RUNTIME_STORAGE_KEY ||
        key === UPLOAD_DRAFT_STORAGE_KEY ||
  ...
  const hasActiveImport =
    liveRuntime?.status === "queued" || liveRuntime?.status === "processing";
  const latestFrames =
    latestActivity?.frames.length ?? latestActivity?.job.frame_count ?? 0;
  const footageDurationSec = latestActivity?.frames.length
    ? Math.max(
        ...latestActivity.frames.map((frame) => frame.timestamp_in_video),
      )
    : null;
  const inspectionCount = history.length;
  const totalFramesExtracted = history.reduce(
    (sum, entry) => sum + (entry.frames.length || 0),
    0,
  );
  const totalFootageDurationSec = history.reduce((sum, entry) => {
    if (!entry.frames.length) return sum;
    const runDuration = Math.max(
      ...entry.frames.map((frame) => frame.timestamp_in_video),
    );
    return sum + runDuration;
  }, 0);
  const footageHours = totalFootageDurationSec
    ? (totalFootageDurationSec / 3600).toFixed(2)
    : null;
  const vesselName =
    latestActivity?.vessel.vessel_name?.trim() || "Unknown vessel";
  const inspectionDate =
    latestActivity?.vessel.inspection_date?.trim() || "Date not set";
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3 lg:items-start">
        <section className="flex min-w-0 flex-col rounded-xl border border-border bg-surface-1 p-6 lg:col-span-2">
          ...
          {hasActiveImport ? (
            <div className="mt-4 rounded-lg border border-accent/40 bg-accent/10 p-4">
              ...
              <p className="mt-2 text-sm text-white">
                Video ID:{" "}
                <span className="font-medium">{liveRuntime.video_id}</span>
              </p>
              <p className="mt-1 text-sm text-muted">
                Status: {liveRuntime.status}
                {typeof liveRuntime.frame_count === "number"
                  ? ` · Frames: ${liveRuntime.frame_count}`
                  : ""}
              </p>
              <div className="mt-4">
                <Link
          ...
          {latestActivity ? (
            <div className="mt-4 rounded-lg border border-border bg-surface-0/60 p-4">
              ...
              <p className="mt-1 text-sm text-muted">
                Status: {latestActivity.job.status} · Frames:{" "}
                {latestActivity.frames.length}
                {footageDurationSec
                  ? ` · Footage duration: ${footageDurationSec.toFixed(1)}s`
                  : ""}
              </p>
            ...
            <>
              <p className="mt-2 shrink-0 text-sm text-muted">
                Completed uploads are saved in this browser. Run an import from
                Upload Footage to see it here.
              </p>
            ...
            <div className="mt-4 rounded-lg border border-border bg-surface-0/40 p-4">
              ...
              <div className="mt-3 space-y-2">
                {history.slice(1, 6).map((entry) => (
                  <div
                    key={entry.video_id}
                    className="flex items-center justify-between gap-3 rounded-md border border-border/70 bg-surface-1/70 px-3 py-2"
                  >
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-muted">
                        {entry.vessel.inspection_date || "Date not set"} ·{" "}
        ...
        <section className="flex flex-col rounded-xl border border-border bg-surface-1 p-6">
          ...
          <div className="mt-5 grid grid-cols-1 gap-2">
            <Link
              className="rounded-lg border border-input-border bg-input-bg px-3 py-2.5 text-center text-sm font-medium text-input-text hover:bg-surface-1 transition-colors"
              to={ROUTES.upload}
            >
              New Upload
            </Link>
            <Link
              ...
              to={
                latestActivity
                  ? ROUTES.analysisByVideo(latestActivity.video_id)
                  : ROUTES.analysis
              }
